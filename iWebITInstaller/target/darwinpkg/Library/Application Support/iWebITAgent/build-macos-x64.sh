...
copyBuildDirectory() {
    sed -i '' -e 's/2.4.7.0/'${VERSION}'/g' "${TARGET_DIRECTORY}/darwin/scripts/postinstall"
    sed -i '' -e 's/iWebITAgent/'${PRODUCT}'/g' "${TARGET_DIRECTORY}/darwin/scripts/postinstall"
    chmod -R 755 "${TARGET_DIRECTORY}/darwin/scripts/postinstall"

    sed -i '' -e 's/2.4.7.0/'${VERSION}'/g' "${TARGET_DIRECTORY}/darwin/Distribution"
    sed -i '' -e 's/iWebITAgent/'${PRODUCT}'/g' "${TARGET_DIRECTORY}/darwin/Distribution"
    chmod -R 755 "${TARGET_DIRECTORY}/darwin/Distribution"

    sed -i '' -e 's/2.4.7.0/'${VERSION}'/g' "${TARGET_DIRECTORY}"/darwin/Resources/*.html
    sed -i '' -e 's/iWebITAgent/'${PRODUCT}'/g' "${TARGET_DIRECTORY}"/darwin/Resources/*.html
    chmod -R 755 "${TARGET_DIRECTORY}/darwin/Resources/"
    ...
    mkdir -p "${TARGET_DIRECTORY}/darwinpkg/Library/Application Support/com.rdfonseca.iWebIT"
    cp -a "$SCRIPTPATH"/application/. "${TARGET_DIRECTORY}/darwinpkg/Library/Application Support/com.rdfonseca.iWebIT"
    chmod -R 755 "${TARGET_DIRECTORY}/darwinpkg/Library/Application Support/com.rdfonseca.iWebIT"
...
function createUninstaller(){
    ...
    sed -i '' -e "s/2.4.7.0/${VERSION}/g" "${TARGET_DIRECTORY}/darwinpkg/Library/${PRODUCT}/${VERSION}/uninstall.sh"
    sed -i '' -e "s/iWebITAgent/${PRODUCT}/g" "${TARGET_DIRECTORY}/darwinpkg/Library/${PRODUCT}/${VERSION}/uninstall.sh"
}
